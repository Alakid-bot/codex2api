...
// NextImageExcluding selects an image-capable account without applying the
// normal per-account concurrent-request window. Image generation is already
// protected by upstream health/cooldown state and API-key/scope policy; the
// account slot is only retained as an accounting counter for release and
// scheduler visibility. This keeps image bursts from being serialized behind
// the general chat concurrency setting.
func (s *Store) NextImageExcluding(apiKeyID int64, exclude map[int64]bool, filter AccountFilter) *Account {
	if s == nil {
		return nil
	}
	...
	sort.SliceStable(accounts, func(i, j int) bool {
		...
		lp, rp := left.schedulerPriority(), right.schedulerPriority()
		if lp != rp {
			return lp > rp
		}
		lt, rt := tierPriority(AccountHealthTier(left.GetHealthTier())), tierPriority(AccountHealthTier(right.GetHealthTier()))
		if lt != rt {
			return lt > rt
		}
		ls, rs := left.GetDispatchScore(), right.GetDispatchScore()
		if ls != rs {
			return ls > rs
		}
		return accountOccupiedRequests(left) < accountOccupiedRequests(right)
	})
	for _, acc := range accounts {
		if acc == nil || exclude != nil && exclude[acc.DBID] || !acc.IsAvailable() {
			continue
		}
		if !s.accountAllowedForAPIKey(acc, apiKeyID) || filter != nil && !filter(acc) {
			continue
		}
		if s.accountHasBlockingCachedCooldown(acc, DispatchPolicyStandard) {
			continue
		}
		if acquired, _ := s.tryAcquireAccountWithFailure(acc, math.MaxInt64, false); acquired {
			return acc
		}
	}
	return nil
...
// TakeImageAccount acquires a specific account for an image retry while
// bypassing the normal per-account concurrency window.
func (s *Store) TakeImageAccount(id, apiKeyID int64, exclude map[int64]bool, filter AccountFilter) *Account {
	if s == nil || id <= 0 || exclude != nil && exclude[id] {
		return nil
	}
	s.mu.RLock()
	acc := s.lookupByIDLocked(id)
	s.mu.RUnlock()
	if acc == nil || !acc.IsAvailable() || !s.accountAllowedForAPIKey(acc, apiKeyID) {
		return nil
	}
	filter = s.withUsableEgressFilter(filter)
	if filter != nil && !filter(acc) || s.accountHasBlockingCachedCooldown(acc, DispatchPolicyStandard) {
		return nil
	}
	if acquired, _ := s.tryAcquireAccountWithFailure(acc, math.MaxInt64, false); acquired {
		return acc
